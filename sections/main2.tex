...
conventions of \cite{sen2026negotiations} for physical quantities and of \cite{sen2026consent} for negotiation objects.
...
\cite{sen2026negotiations}, user $v$'s visits are indexed by sessions $k \in \Kv =
...
\end{equation}
which is the persistence coupling of \cite{sen2026negotiations}; as in that
...
\banked{v}{k} \;=\; \Edep(v,k) - \Eneg(v,k) \;\ge\; 0
\quad \text{(kWh)},
\end{equation}
which is nonnegative by the floor guarantee \eqref{eq:floor}. Of this
excess, the portion still present in the battery at the next arrival is the
survived excess,
\begin{equation}\label{eq:survived}
\survived{v}{k{+}1} \;=\; \min\bigl(\banked{v}{k},\;
\Earr(v,k{+}1)\bigr) \quad \text{(kWh)};
\end{equation}
the excess survives in full unless off-site consumption draws the battery
below the banked amount, in which case only the energy present at arrival
is attributable to the over-charge. The per-user yield is
...
\subseteq \Td$; events are exogenous, sparse, and
program-dependent. The capacity payment and the violation penalty are
\begin{equation}\label{eq:dr}
\Cinc{d} = \winc \cdot (\Phist - \FSL_d), \qquad
\Cpen{d} = \Kfsl \sum_{t \in \evwin{d}} \vio{t}\, \dt,
\end{equation}
where $\Phist$ is the program's historical baseline, $\winc$ (\$/kW) is the
per-day-normalized capacity rate, $\vio{t} = \max(0, \Ptot{t} - \FSL_d)$ is
the violation slack, and $\Kfsl$ (\$/kWh) is the penalty rate. The pledge constitutes the only commitment in the system; no
...
to $\winc$ per pledged day. Results under an enrollment-fixed FSL are a
constrained special case ($\FSL_d \equiv \FSL$ for all $d$) and are
reported in the sensitivity analysis.
\end{remark}

\noindent\textbf{Building's Policies.}  
Let \(\pi^{neg}\) and \(\pi^{ch}\) denote the negotiation and charging policies, respectively, and define the combined policy as \(\pi = (\pi^{neg}, \pi^{ch})\). The building’s optimal policy \(\pi^* = (\pi^{neg,*}, \pi^{ch,*})\) jointly determines negotiation offers and EV charging schedules to minimize total costs, including operational expenses and user incentives, at each decision point.

\noindent\textbf{User's Marginal Utility.}
$\boldsymbol\theta_v$ denotes user $v$'s arrival and departure choices. Let $\boldsymbol{\theta}_{\setminus v}$ be the realized choices of all users who have arrived prior to $v$. For users arriving after \(v\), let \(\boldsymbol{\hat{\theta}}_{\setminus v}\) be random variables with joint distribution \(\psi\), representing all possible future arrivals along with their associated choices and flexibilities. Instead of point forecasts, the controller samples from \(\psi\) to estimate expected system costs under uncertainty, as \(\psi\) encompasses the stochastic processes of EV arrivals and user decisions (e.g., arrival/departure times and flexibility preferences).
...
\begin{equation}\label{eq:user_utility}
 % \small
\begin{split}
U^{\boldsymbol{\theta}_{\setminus v}, \pi^{*}}(\theta_v) = \mathbb{E}_{\boldsymbol{\hat{\theta}}_{\setminus v} \sim \psi}
    \Big[
...
\end{split}
\end{equation}  
where the expectation integrates over all future scenarios varying in arrival numbers, user types, and their flexibility parameters. We assume the quantity \(\mathcal{W}'\) represents building loads, energy charges, and demand charge parameters.
...
\paragraph{User cost and payments.} User $v$'s cost for session $k$
retains the two-term form of \cite{sen2026consent}:
\begin{equation}\label{eq:usercost}
\ucost{k}{v} \;=\; \sum_{t \in \Tvk} \we{t}\, \gch{t}{v} \;-\;
\alpha \cdot \Umu{\theta_v},
\end{equation}
where $\gch{t}{v}$ is the energy charged to $v$ (negative for discharge)
and $\alpha \in [0,1]$ is the cost-sharing parameter. The compensation
term generalizes CONSENT's marginal utility to the user's marginal
contribution under the FSL-constrained system: $\Umu{\theta_v}$ comprises
(i) the expected cost difference attributable to the negotiated
flexibility \cite{sen2026consent}, evaluated under the constraint set of
Layer~2, and (ii) for sessions that overlap an event window, the realized
cost difference attributable to discharging $v$'s surviving buffer,
evaluated ex post by counterfactual replay of the day with and without
$\survived{v}{k}$. No separate event-payment term is required: the energy
credit for discharged quantities is carried by the first term through
$\gch{t}{v} < 0$, and because the building cost includes the degradation
term $\Kbatt$, the replay difference is automatically net of battery wear.
The banked excess itself is provided at no cost: $\banked{v}{k}$ does not
appear in \eqref{eq:usercost}. Every term is settled on realized
quantities; no term depends on a forecast of user $v$'s behavior.
...
this layer. The pledge is a function only of information available a day
ahead: forecasts of building load, arrivals, and event likelihood, together
with the forecast realizable buffer $\buffhat{d}$, constructed from
\eqref{eq:buffer} under the current acceptance and yield estimates
$(\ohat, \rhohat)$. In particular, $\FSL_d$ may not depend on any quantity
realized during day $d$. The pledge maximizes expected capacity revenue net
of expected penalty and delivery cost (Section~4.1). The commitment and its
...
MC-MPC of \cite{sen2026negotiations}, with the pledge entering as a constraint,
...
schedules buffer discharge and evaluates the realized component of
$\Umu{\theta_v}$ by counterfactual replay.
...
+ \sum_{v,k} \alpha\, \Umu{\theta_v}
\Bigr],
\end{aligned}
\end{equation}
where $\zund{k}{v}$ denotes unmet floor energy and $\qdis{t}{v}$ denotes
discharged energy, mirroring \cite{sen2026consent} augmented with the DR terms
...
mechanism must preserve voluntary participation and budget feasibility \cite{sen2026consent}, and we require that no payment, obligation, or
...
inherited from \cite{sen2026consent} for the negotiation step; Section~4.5
...
and negotiated agreements $\{\Eneg, \tdep, \zeta^v\}$, jointly
satisfying the participation and budget guarantees.