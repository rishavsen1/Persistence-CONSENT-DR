...
problem given $v$'s report, and let $G$ be any cost functional (left arbitrary so that the result holds whether or not the day-$d$ cost includes the event and DR terms). If
...
\]
where $c^{\mathrm{next}}_{v}$ is the \emph{realized} next-session marginal cost and $\cmarg{v}$ is its \emph{forecast} \eqref{eq:pbank}, so $\epsIC{v}{d}$ is the (positive part of the) forecast error in the next-session price, scaled by the banked excess $\banked{v}{k}$. This $\epsIC{v}{d}$ is unrelated to the delivery risk level $\varepsilon$ of Section~\ref{sec:formulation}; it is a per-user, per-day bound, not an operator tolerance. If forecasts are exact ($\cmarg{v} = c^{\mathrm{next}}_{v}$), $\epsIC{v}{d} = 0$ and the mechanism is exactly periodic ex-post IC; if additionally $v$ has a single session, the statement
reduces to CONSENT's Theorem~1.
\end{theorem}

\begin{proof}
\emph{Step 1 (report channel).} Reporting
$\bar{t}_{\mathrm{req}} < \treq(v,k)$ requires any feasible schedule to set
...
reassignment effects). Reporting $\bar{E}_{\mathrm{req}} > \Ereq(v,k)$
...
reportable from the forecast diagnostics of Section~\ref{sec:eval}.
\end{remark}
...
Let $\Ubuf(v,k)$ and $\Udc(v,k)$ be the replay shares
\eqref{eq:ubuf}--\eqref{eq:udc} and let the paid amount be the capped
combined marginal $\min\bigl(\Ubuf(v,k)+\Udc(v,k),\ \text{proportional
share}\bigr)$ of Section~\ref{sec:negotiation}. Then
...
are paid. Because $\Ubuf+\Udc$ is a partition of the single replay
difference \eqref{eq:ubuf}, the demand-charge and energy-plus-penalty
shares are jointly bounded by this one saving; $\pmaxre$ being a $\max$
over disjoint steps is itself submodular, so on non-event days the
$\Udc$-only payment inherits the same guarantee against the realized
demand-charge reduction $\wdr{}(\pmaxre(\emptyset) - \pmaxre(\buff{d}))$.
\end{lemma}
...
operating range of $\FSL$ is verified numerically in Section~\ref{sec:eval}. The
...
For any fixed scenario drawn from $\psihat$, additional dischargeable
energy can only relax the delivery constraint $\Ptot{t} \le \FSL$ on
$\evwin{d}$: the controller retains every feasible charging plan it had
with the smaller buffer $\buffhat{d}$ and gains new ones, so the
per-scenario minimum deliverable $\FSL$ is nonincreasing in the buffer.
Taking the probability over scenarios preserves the inequality, so the
chance constraint \eqref{eq:pledgeset} at $\buffhat{d}'$ is satisfied
by a superset of commitments, giving $\Omeps(\buffhat{d}) \subseteq
\Omeps(\buffhat{d}')$ and $\min\Omeps(\buffhat{d}') \le
\min\Omeps(\buffhat{d})$. The revenue statement follows from
\eqref{eq:dr}, decreasing in $\FSL_d$. The argument assumes the extra
buffer is dischargeable through bidirectional-assigned capacity within
$\evwin{d}$; buffer stranded on unidirectional chargers does not enter
$\buff{d}$ by construction \eqref{eq:buffer}, so the monotonicity is
with respect to the deliverable buffer only.
\end{proof}
...
$\{\etarget{v}{k}\}$ of \eqref{eq:bshort}. Then: (a) $\sigma_d$ is a
...
problem; setting $\etarget{v}{k}$ to its optimal pre-event departure SoC
...
Section~\ref{sec:eval} reports numerically.
\end{proof}
